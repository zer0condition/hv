...
int vmcs_setup_host(struct cpu_ctx *cpu)
{
    ...
    host_rsp = (u64)&cpu->host_stack->cpu;
...
{
    struct hv_cpu_state *state = &cpu->state;
    u64 guest_cr0, guest_cr4;
    u64 fixed0, fixed1;
    int err = 0;
    
    guest_cr0 = state->cr0.AsUInt;
    fixed0 = native_read_msr(IA32_VMX_CR0_FIXED0);
    fixed1 = native_read_msr(IA32_VMX_CR0_FIXED1);
    guest_cr0 |= fixed0;
    guest_cr0 &= fixed1;
    
    guest_cr4 = state->cr4.AsUInt;
    fixed0 = native_read_msr(IA32_VMX_CR4_FIXED0);
    fixed1 = native_read_msr(IA32_VMX_CR4_FIXED1);
    
    hv_cpu_log(debug, "cr4 fixup: original=0x%llx fixed0=0x%llx fixed1=0x%llx\n",
               guest_cr4, fixed0, fixed1);
    
    guest_cr4 |= fixed0;
    guest_cr4 &= fixed1;
    
    hv_cpu_log(debug, "cr4 after fixup: 0x%llx\n", guest_cr4);
    
    err |= arch_vmwrite(VMCS_GUEST_CR0, guest_cr0);
    err |= arch_vmwrite(VMCS_GUEST_CR3, state->cr3.AsUInt);
    err |= arch_vmwrite(VMCS_GUEST_CR4, guest_cr4);
    
    {
        u64 readback = arch_vmread(VMCS_GUEST_CR4);
        hv_cpu_log(debug, "cr4 vmread after write: 0x%llx\n", readback);
    }
    ...
    err |= arch_vmwrite(VMCS_GUEST_ACTIVITY_STATE, 0);
